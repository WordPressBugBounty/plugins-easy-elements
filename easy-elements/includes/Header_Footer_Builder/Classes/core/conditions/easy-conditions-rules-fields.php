...
class EASY_EHF_Target_Rules_Fields {
	...
	public static function target_rule_settings_field( $name, $settings, $value ) {
		...
		$add_rule_label = isset( $settings['add_rule_label'] ) ? $settings['add_rule_label'] : esc_html__( 'Add Rule', 'easy-elements' );
		$saved_values   = $value;
		$output         = '';
		...
		*/
		...
		$output .= '<option value="">' . esc_html__( 'Select', 'easy-elements' ) . '</option>';
		...
		foreach ( $selection_options as $group => $group_data ) {
			...
			foreach ( $group_data['value'] as $opt_key => $opt_value ) {
				$output .= '<option value="' . esc_attr( $opt_key ) . '">' . wp_kses_post( $opt_value ) . '</option>';
			}
			$output .= '</optgroup>';
		}
		$output .= '</select>';
		...
		/* Wrapper Start */
		$output .= '<div class="easyel-target-rule-wrapper ast-target-rule-' . esc_attr( $rule_type ) . '-on-wrap" data-type="' . esc_attr( $rule_type ) . '">';
		
		$output .= '<div class="easyel-target-rule-selector-wrapper ast-target-rule-' . esc_attr( $rule_type ) . '-on">';
	...
	public static function easy_generate_target_rule_selector( $type, $selection_options, $input_name, $saved_values, $add_rule_label ) {
		...
		foreach ( $saved_values['rule'] as $index => $data ) {
			$output .= '<div class="easyel-target-rule-condition ast-target-rule-' . esc_attr( $index ) . '" data-rule="' . esc_attr( $index ) . '" >';
			/* Condition Selection */
			$output .= '<span class="target_rule-condition-delete dashicons dashicons-dismiss"></span>';
			$output .= '<div class="target_rule-condition-wrap" >';
			...
			foreach ( $selection_options as $group => $group_data ) {
				...
				foreach ( $group_data['value'] as $opt_key => $opt_value ) {
					...
					$output .= '<option value="' . esc_attr( $opt_key ) . '" ' . esc_attr( $selected ) . '>' . wp_kses_post( $opt_value ) . '</option>';
				}
				$output .= '</optgroup>';
			}
			$output .= '</select>';
			...
			if ( 'specifics' == $data && isset( $saved_values['specific'] ) && null != $saved_values['specific'] && is_array( $saved_values['specific'] ) ) {
				foreach ( $saved_values['specific'] as $data_key => $sel_value ) {
					...
					if ( strpos( $sel_value, 'post-' ) !== false ) {
						...
						$output    .= '<option value="post-' . esc_attr( $post_id ) . '" selected="selected" >' . wp_kses_post( $post_title ) . '</option>';
					}
					...
					if ( strpos( $sel_value, 'tax-' ) !== false ) {
						...
						$output .= '<option value="' . esc_attr( $sel_value ) . '" selected="selected" >' . wp_kses_post( $term_name ) . '</option>';
					}
				}
			}
			$output .= '</select>';
		...
		$output .= '<a href="#" class="button" data-rule-id="' . absint( $index ) . '" data-rule-type="' . esc_attr( $type ) . '">' . wp_kses_post( $add_rule_label ) . '</a>';
		$output .= '</div>';

		if ( 'display' == $type ) {
			/* Add new rule */
			$output .= '<div class="target_rule-add-exclusion-rule">';
			$output .= '<a href="#" class="button">' . esc_html__( 'Add Exclusion Rule', 'easy-elements' ) . '</a>';
			$output .= '</div>';
		}
	...
	public static function target_user_role_settings_field( $name, $settings, $value ) {
		...
		foreach ( $saved_values as $index => $data ) {
			$output     .= '<div class="easyel-user-role-condition ast-user-role-' . esc_attr( $index ) . '" data-rule="' . esc_attr( $index ) . '" >';
				$output .= '<span class="user_role-condition-delete dashicons dashicons-dismiss"></span>';
				/* Condition Selection */
				$output     .= '<div class="user_role-condition-wrap" >';
					$output .= '<select name="' . esc_attr( $input_name ) . '[' . esc_attr( $index ) . ']" class="user_role-condition form-control ast-input">';
					$output .= '<option value="">' . esc_html__( 'Select', 'easy-elements' ) . '</option>';

			foreach ( $selection_options as $group => $group_data ) {
				$output .= '<optgroup label="' . $group_data['label'] . '">';
				foreach ( $group_data['value'] as $opt_key => $opt_value ) {
					$output .= '<option value="' . esc_attr( $opt_key ) . '" ' . selected( $data, $opt_key, false ) . '>' . wp_kses_post( $opt_value ) . '</option>';
				}
				$output .= '</optgroup>';
			}
					$output .= '</select>';
				...
				$output .= '<a href="#" class="button" data-rule-id="' . absint( $index ) . '">' . wp_kses_post( $add_rule_label ) . '</a>';
				$output .= '</div>';
			$output     .= '</div>';
		$output         .= '</div>';
	...
	public function get_posts_by_conditions( $post_type, $option ) {
		...
		$post_type = $post_type ?: ( $post->post_type ?? 'post' );
		...
		$option['current_post_id']      = self::$current_page_data['ID'] ?? 0;
		...
		if ( false === $meta_header ) {
			...
			// Default: global
			$where_clauses[]  = 'pm.meta_value LIKE %s';
			$prepare_values[] = '%"basic-global"%';

			// Handle current page type
			switch ( $current_page_type ) {
				case 'is_404':
					$where_clauses[]  = 'pm.meta_value LIKE %s';
					$prepare_values[] = '%"special-404"%';
					break;

				case 'is_search':
					$where_clauses[]  = 'pm.meta_value LIKE %s';
					$prepare_values[] = '%"special-search"%';
					break;
				...
				case 'is_author':
					$where_clauses[]  = 'pm.meta_value LIKE %s';
					$prepare_values[] = '%"basic-archives"%';

					$where_clauses[]  = 'pm.meta_value LIKE %s';
					$prepare_values[] = '%"' . $current_post_type . '|all|archive"%';

					if ( 'is_tax' === $current_page_type && is_object( $q_obj ) ) {
						$where_clauses[]  = 'pm.meta_value LIKE %s';
						$prepare_values[] = '%"' . $current_post_type . '|all|taxarchive|' . sanitize_key( $q_obj->taxonomy ) . '"%';

						$where_clauses[]  = 'pm.meta_value LIKE %s';
						$prepare_values[] = '%"tax-' . intval( $q_obj->term_id ) . '"%';
					} elseif ( 'is_date' === $current_page_type ) {
						$where_clauses[]  = 'pm.meta_value LIKE %s';
						$prepare_values[] = '%"special-date"%';
					} elseif ( 'is_author' === $current_page_type ) {
						$where_clauses[]  = 'pm.meta_value LIKE %s';
						$prepare_values[] = '%"special-author"%';
					}
					break;

				case 'is_home':
					$where_clauses[]  = 'pm.meta_value LIKE %s';
					$prepare_values[] = '%"special-blog"%';
					break;
				...
				case 'is_front_page':
					...
					$where_clauses[]  = 'pm.meta_value LIKE %s';
					$prepare_values[] = '%"special-front"%';

					$where_clauses[]  = 'pm.meta_value LIKE %s';
					$prepare_values[] = '%"' . $current_post_type . '|all"%';

					$where_clauses[]  = 'pm.meta_value LIKE %s';
					$prepare_values[] = '%"post-' . intval( $current_id ) . '"%';
					break;
				...
				case 'is_singular':
					...
					$where_clauses[]  = 'pm.meta_value LIKE %s';
					$prepare_values[] = '%"basic-singulars"%';
					...
					if ( ! empty( $terms ) && ! is_wp_error( $terms ) ) {
						foreach ( $terms as $term ) {
							$where_clauses[]  = 'pm.meta_value LIKE %s';
							$prepare_values[] = '%"tax-' . intval( $term->term_id ) . '-single-' . sanitize_key( $term->taxonomy ) . '"%';
						}
					}
					break;
				...
				case 'is_woo_shop_page':
					$where_clauses[]  = 'pm.meta_value LIKE %s';
					$prepare_values[] = '%"special-woo-shop"%';
					break;
			...
			$all_args = array_merge( [ $location, $post_type ], $prepare_values );

			$cache_key = 'ehf_posts_' . md5( $sql . implode( ',', $all_args ) );
			$posts     = wp_cache_get( $cache_key );

			if ( false === $posts ) {
				// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.PreparedSQL.NotPrepared
				$prepared_sql = $wpdb->prepare( $sql, ...$all_args );
			...
			$option['current_post_id'] = $current_post_id ?? false;
		...
		if ( ! empty( $already_set_rule ) ) {
			add_action(
				...
				function() use ( $already_set_rule ) {
					...
					$notice = sprintf( esc_html__( 'The same display setting is already exist in %s post/s.', 'easy-elements' ), $rule_set_titles );
